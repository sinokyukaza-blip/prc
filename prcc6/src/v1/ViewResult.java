...
public class ViewResult implements View {
    ...
    /**
     * Sorts the results by average.
     */
    public void sortByAverage() {
        results.sort((r1, r2) -> Double.compare(r1.getAverage(), r2.getAverage()));
    }

    /**
     * Finds the result with max ones count.
     * @return the result or null if empty
     */
    public ResultData findMaxOnes() {
        if (results.isEmpty()) return null;
        ResultData max = results.get(0);
        for (ResultData r : results) {
            if (r.getOnesCount() > max.getOnesCount()) {
                max = r;
            }
        }
        return max;
